...
from .models import User as DBUser, Profile as DBProfile, Bookmark as DBBookmark
...
def get_user_by_phone_number(db: Session, phone_number: str):
    return db.query(DBUser).filter(DBUser.profile.has(phone_number=phone_number)).first()
...
def remove_bookmark(db: Session, bookmark_id: int, user_id: int):
    db_bookmark = db.query(DBBookmark).filter(DBBookmark.id == bookmark_id, DBBookmark.user_id == user_id).first()
    if db_bookmark:
