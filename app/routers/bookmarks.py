...
from ..crud import add_bookmark, get_user_bookmarks, get_user_by_username, remove_bookmark
...
def add_new_bookmark(value: str, type: str, token: str = Depends(verify_token), db: Session = Depends(get_db)):
    user_id = get_user_by_username(db, token).id
    return add_bookmark(db, user_id, value, type)

@router.delete("/bookmarks/{bookmark_id}", response_model=Bookmark)
def remove_existing_bookmark(bookmark_id: int, token: str = Depends(verify_token), db: Session = Depends(get_db)):
    user_id = get_user_by_username(db, token).id
